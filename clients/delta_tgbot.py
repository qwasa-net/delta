...
def setup_logger(debug=False):
    ...
    logging.basicConfig(
        ...
        format="[%(asctime)-15s] %(levelname)s [%(process)d#%(module)s.%(funcName)s]: %(message)s",
    )
...
class DeltaTG:
    ...
    async def webhook(self, request):
        ...
        from_name = (
            " ".join(filter(None, (msg_from.get("first_name"), msg_from.get("last_name"))))
            or from_username
            or from_id
        )
        text = msg.get("text")

        logging.info("(%s) message from #%s (%s %s): `%s`", i, from_id, from_username, from_name, text)
    ...
    def try_quick_reply(self, text, from_name, from_username, chat_id):
        ...
        if self.START_REPLY and (not text or (self.START_INPUT and text == self.START_INPUT)):
    ...
    def send_tg_message(chat_id, text):
        ...
        data = {"chat_id": chat_id, "text": text[:4096], "parse_mode": "Markdown"}  # TG API limit
...
def main():
    ...
    logging.info(
        "starting -- at %s:%s%s, with %s dicts, %s workers, using `%s…` token",
        args.host,
        args.port,
        args.webhook_url,
        len(args.dictionary),
        args.max_workers or "few",
        args.api_token[:8],
    )
